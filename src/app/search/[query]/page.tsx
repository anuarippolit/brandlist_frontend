...
//           fetchMoreProducts(); // Will always use latest version
//         }
//       },
//       { threshold: 1 }
//     );
...
//     };
//   }, [loaderRef.current]); // Only set up once — doesn’t depend on fetchMoreProducts
...
const SearchResults = ({ params }: { params: Promise<{ query: string }> }) => {
  ...
  // ✅ Single IntersectionObserver that doesn't depend on unstable deps
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
  ...
  }, [fetchMoreProducts]); // this is fine — because it's stable from useCallback
